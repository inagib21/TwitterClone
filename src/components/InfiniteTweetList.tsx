...
export function InfiniteTweetList({ 
  ...
  function TweetCard({id, user, content, createdAt, likeCount, likedByMe}:Tweet){
    ...
    return <li className="flex gap-4 border-4 px-4 py-4">
        <Link href={`/profiles/${user.id}`}>
            <ProfileImage src = {user.image} />
    </Link>
    <div className="flex flex-grow flex-col">
       <div className="flex gap-1">
        <Link href={`/profiles/${user.id}`}className="font-bold outline:none
  ...
  function HeartButton({ isLoading, onClick, likedByMe, likeCount }: HeartButtonProps) {
    ...
    return (
      <button
        onClick={onClick}
        disabled={isLoading}
        className={`group -ml-2 items-center gap-1 self-start flex
        ...
        <IconHoverEffect red>
          <HeartIcon
            className={`transition-colors duration-200 ${likedByMe ?
              "opacity-80" : "group-hover:opacity-80 group-focus-visible:opacity-80"
            }`}
          />
        </IconHoverEffect>
        <span>{likeCount}</span>
